...
from app.services.azure.checks.nic import check_no_public_ip, check_ip_forwarding_disabled
...
def _make_asset(resource_type="microsoft.network/networkinterfaces", raw_properties=None):
    return Asset(id=uuid.uuid4(), cloud_account_id=uuid.uuid4(),
        ...
        resource_type=resource_type, name="test-nic", region="westeurope",
...
class TestCheckNoPublicIp:
    ...
    def test_fail_when_public_ip_attached(self):
        asset = _make_asset(raw_properties={"ipConfigurations": [
            {"properties": {"publicIPAddress": {"id": "/sub/pip1"}}}]})
