...
from app.services.azure.checks.network import (
    ...
    check_public_ip_ddos,
    check_vnet_ddos,
    check_vpn_gateway_sku,
)
...
class TestCheckVnetDdos:
    def test_pass_when_enabled(self):
        asset = _make_asset(
            resource_type="microsoft.network/virtualnetworks", raw_properties={"enableDdosProtection": True}
        )
        assert check_vnet_ddos(asset).status == "pass"

    def test_fail_when_disabled(self):
        asset = _make_asset(
            resource_type="microsoft.network/virtualnetworks", raw_properties={"enableDdosProtection": False}
        )
...
class TestCheckNetworkWatcherEnabled:
    def test_pass_when_succeeded(self):
        asset = _make_asset(
            resource_type="microsoft.network/networkwatchers", raw_properties={"provisioningState": "Succeeded"}
        )
        assert check_network_watcher_enabled(asset).status == "pass"

    def test_fail_when_failed(self):
        asset = _make_asset(
            resource_type="microsoft.network/networkwatchers", raw_properties={"provisioningState": "Failed"}
        )
...
class TestCheckVpnGatewaySku:
    def test_pass_when_vpngw1(self):
        asset = _make_asset(
            resource_type="microsoft.network/virtualnetworkgateways", raw_properties={"sku": {"name": "VpnGw1"}}
        )
        assert check_vpn_gateway_sku(asset).status == "pass"

    def test_fail_when_basic(self):
        asset = _make_asset(
            resource_type="microsoft.network/virtualnetworkgateways", raw_properties={"sku": {"name": "Basic"}}
        )
