"""Unit tests for Batch Account checks."""
from __future__ import annotations
import uuid
from datetime import UTC, datetime
from app.models.asset import Asset
from app.services.azure.checks.batch import check_public_access, check_encryption

def _make_asset(resource_type="microsoft.batch/batchaccounts", raw_properties=None):
    return Asset(id=uuid.uuid4(), cloud_account_id=uuid.uuid4(),
        provider_id=f"/subscriptions/{uuid.uuid4().hex}/resourceGroups/test/providers/{resource_type}/testbatch",
        resource_type=resource_type, name="test-batch", region="westeurope",
...
class TestCheckEncryption:
    def test_pass_when_cmk(self):
        ...
        assert check_encryption(asset).status == "pass"
    def test_fail_when_service_managed(self):
        asset = _make_asset(raw_properties={"encryption": {"keySource": "Microsoft.Batch"}})
        assert check_encryption(asset).status == "fail"
    def test_fail_when_property_missing(self):
        asset = _make_asset(raw_properties={})
        assert check_encryption(asset).status == "fail"
    def test_fail_when_raw_properties_none(self):
