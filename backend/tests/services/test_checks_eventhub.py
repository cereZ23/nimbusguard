"""Unit tests for Event Hub checks."""
from __future__ import annotations
import uuid
from datetime import UTC, datetime
from app.models.asset import Asset
from app.services.azure.checks.eventhub import check_cmk_encryption, check_public_access

def _make_asset(resource_type="microsoft.eventhub/namespaces", raw_properties=None):
    return Asset(id=uuid.uuid4(), cloud_account_id=uuid.uuid4(),
        provider_id=f"/subscriptions/{uuid.uuid4().hex}/resourceGroups/test/providers/{resource_type}/testeh",
        resource_type=resource_type, name="test-eh", region="westeurope",
...
class TestCheckCmkEncryption:
    ...
    def test_fail_when_service_managed(self):
        asset = _make_asset(raw_properties={"encryption": {"keySource": "Microsoft.EventHub"}})
