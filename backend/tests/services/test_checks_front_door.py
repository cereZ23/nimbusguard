...
from app.services.azure.checks.front_door import check_https_redirect, check_waf_policy
...
class TestCheckWafPolicy:
    def test_pass_when_waf_attached(self):
        asset = _make_asset(
            raw_properties={
                "frontendEndpoints": [{"properties": {"webApplicationFirewallPolicyLink": {"id": "/sub/waf1"}}}]
            }
        )
...
class TestCheckHttpsRedirect:
    ...
    def test_fail_when_no_redirect(self):
        fwd_type = "#Microsoft.Azure.FrontDoor.Models.FrontdoorForwardingConfiguration"
        asset = _make_asset(
            raw_properties={"routingRules": [{"properties": {"routeConfiguration": {"@odata.type": fwd_type}}}]}
        )
