"""Unit tests for Redis Cache checks."""
from __future__ import annotations
import uuid
from datetime import UTC, datetime
from app.models.asset import Asset
from app.services.azure.checks.redis import check_min_tls_version, check_public_access, check_non_ssl_port_disabled

def _make_asset(resource_type="microsoft.cache/redis", raw_properties=None):
    return Asset(id=uuid.uuid4(), cloud_account_id=uuid.uuid4(),
        provider_id=f"/subscriptions/{uuid.uuid4().hex}/resourceGroups/test/providers/{resource_type}/testredis",
        resource_type=resource_type, name="test-redis", region="westeurope",
...
class TestCheckMinTlsVersion:
    def test_pass_when_tls_12(self):
        asset = _make_asset(raw_properties={"minimumTlsVersion": "1.2"})
        assert check_min_tls_version(asset).status == "pass"
    def test_fail_when_tls_10(self):
        asset = _make_asset(raw_properties={"minimumTlsVersion": "1.0"})
...
class TestCheckNonSslPortDisabled:
    def test_pass_when_disabled(self):
        asset = _make_asset(raw_properties={"enableNonSslPort": False})
        assert check_non_ssl_port_disabled(asset).status == "pass"
    def test_fail_when_enabled(self):
        asset = _make_asset(raw_properties={"enableNonSslPort": True})
        assert check_non_ssl_port_disabled(asset).status == "fail"
