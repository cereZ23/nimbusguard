...
from app.services.azure.checks.log_analytics import check_retention_days, check_cmk_encryption
...
def _make_asset(resource_type="microsoft.operationalinsights/workspaces", raw_properties=None):
    return Asset(id=uuid.uuid4(), cloud_account_id=uuid.uuid4(),
        ...
        resource_type=resource_type, name="test-la", region="westeurope",
...
class TestCheckCmkEncryption:
    ...
    def test_pass_when_key_vault(self):
        asset = _make_asset(raw_properties={"encryption": {"keyVaultProperties": {"keyVaultUri": "https://kv.vault.azure.net"}}})
        assert check_cmk_encryption(asset).status == "pass"
